...
| Vendoring `devkit_Regex.bas` with the engine's full API left `Public` | `make_aio` re-publishes API names including `Replace`; even under `Option Private Module` a Public member is project-global for *unqualified* resolution, so it shadows VBA's built-in `Replace` and breaks unrelated code (`DevkitApplyFixture`'s `Replace(md, ...)`) with a compile-time "ByRef argument type mismatch" -- surfaced as an invisible modal that hangs the headless run | Demote every engine API to `Private` except the three functions devkit_Test calls (`InitializeRegex` / `TryInitializeRegex` / `Test`); re-apply this after any regen (documented in the file header) |
| Returning a helper's list bare (`return @($out)`) and reading `.Count` at the call site | PowerShell unwraps a 0-element result to `$null` and a 1-element result to a scalar; under `Set-StrictMode -Version Latest`, `$null.Count` throws "property 'Count' cannot be found" | Wrap every call site in `@(...)` so the result is always an array |
| Fixpoint shrink loop that re-tries each input against both neutral values (blank, `0`) | When *both* neutrals preserve a failure, a position oscillates blank<->0 forever, draining the shared `-MaxShrinkSteps` budget on the first failing case so later cases never shrink (raising the budget does not help -- the loop just spins more) | Single greedy pass: finalise each position once to the most-neutral preserving value (`$neutrals` ordered most-neutral first; `break` once the current value already equals a candidate) |
